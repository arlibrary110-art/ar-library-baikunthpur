...
if (in_array($action, ['add_attendance'], true)) require_permission('attendance');
if (in_array($action, ['seat','save_seat'], true)) require_permission('seats');

// Birthday reminder support: keep existing databases compatible.
...
if ($action === "list") {
    ...
    $conn->query("
        ...
        SET status =
            CASE
                WHEN validity_date IS NOT NULL
...
if ($action === "add") {
    ...
    |---------------------------------------------------------
    | A newly registered member is immediately added to the
    | Due Fee list. The amount is taken from fee_settings so
    | Fee Setting changes are respected automatically.
    */
    $feeTable = "
    ...
    /* member_fees is created above in this API, so the pending record can be inserted now. */
