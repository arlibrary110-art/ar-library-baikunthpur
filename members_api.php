...
if ($action === "renew") {
    ...
    $termKey=$months===12?'twelve':($months===6?'six':($months===3?'three':'one'));
    $feeKey=$feePlanKey.'_'.$termKey;
    $feeAmount=0;
    ...
    if($feeAmount<=0){
        $monthlyFallback=['half_day'=>600,'half_reserved'=>800,'full_day'=>1100,'full_reserved'=>1300];
        $feeAmount=($monthlyFallback[$feePlanKey]??1100) * $months;
        if($months===3)$feeAmount*=0.95;
        if($months===6)$feeAmount*=0.90;
        if($months===12)$feeAmount*=0.90;
    }

    $conn->begin_transaction();
    try {
        $up=$conn->prepare("UPDATE members SET validity_date=?, status='Active', membership_plan=?, shift=? WHERE id=?");
        $plan=$months===1?'1 Month':($months===3?'3 Months':($months===6?'6 Months':'12 Months'));
