...
if (in_array($action, ['list','get','add','renew','update','delete','create_initial_due'], true)) require_permission('members');
...
/* =========================================================
   CREATE INITIAL FEE DUE (called after member creation)
   This is intentionally separate from member creation so a fee-ledger
   problem can never make the UI report "Could not save member" after the
   member has already been created.
========================================================= */
if ($action === "create_initial_due") {
    $memberId = (int)($_POST["member_id"] ?? 0);
    if ($memberId <= 0) jsonResponse(["success"=>false,"message"=>"Invalid member ID."],400);

    $m = $conn->prepare("SELECT id, member_id, membership_plan, shift, joining_date FROM members WHERE id = ? LIMIT 1");
    if (!$m) jsonResponse(["success"=>false,"message"=>"Could not prepare member lookup: ".$conn->error],500);
    $m->bind_param("i", $memberId);
    if (!$m->execute()) { $e=$m->error; $m->close(); jsonResponse(["success"=>false,"message"=>"Could not load member: ".$e],500); }
    $member = $m->get_result()->fetch_assoc(); $m->close();
    if (!$member) jsonResponse(["success"=>false,"message"=>"Member not found."],404);

    // Idempotent: never create a second initial pending due for the same member.
    $existing = $conn->prepare("SELECT id, amount, status FROM member_fees WHERE member_id = ? AND status = 'Pending' ORDER BY id DESC LIMIT 1");
    if ($existing) {
        $existing->bind_param("i", $memberId); $existing->execute();
        $row = $existing->get_result()->fetch_assoc(); $existing->close();
        if ($row) jsonResponse(["success"=>true,"message"=>"Fee due already exists.","id"=>(int)$row['id'],"amount"=>(float)$row['amount'],"status"=>$row['status']]);
    }
    ...
    $feePlan = strpos($shiftKey,'full day') !== false ? (strpos($shiftKey,'reserved')!==false?'full_reserved':'full_day') : (strpos($shiftKey,'reserved')!==false?'half_reserved':'half_day');
    $term = strpos($planKey,'6 month')!==false ? 'six' : (strpos($planKey,'3 month')!==false ? 'three' : 'one');
    $feeKey = $feePlan.'_'.$term;
    $amount = (float)($defaults[$feeKey] ?? 0);
    $fs = $conn->prepare("SELECT setting_value FROM fee_settings WHERE setting_key = ? LIMIT 1");
    if ($fs) { $fs->bind_param("s",$feeKey); if($fs->execute()){ $fr=$fs->get_result()->fetch_assoc(); if($fr && isset($fr['setting_value'])) $amount=(float)$fr['setting_value']; } $fs->close(); }
    if ($amount <= 0) jsonResponse(["success"=>false,"message"=>"Fee amount is not configured for this plan/shift."],500);

    $date = !empty($member['joining_date']) ? $member['joining_date'] : date('Y-m-d');
    $receipt = "DUE-".$member['member_id']."-".date('YmdHis')."-".random_int(100,999);
    $remarks = "Initial registration fee due";
    $st = $conn->prepare("INSERT INTO member_fees (member_id, receipt_no, amount, payment_date, due_date, payment_method, status, remarks) VALUES (?, ?, ?, ?, ?, 'Pending', 'Pending', ?)");
    if (!$st) jsonResponse(["success"=>false,"message"=>"Could not prepare fee due: ".$conn->error],500);
    $st->bind_param("isdsss",$memberId,$receipt,$amount,$date,$date,$remarks);
    if (!$st->execute()) { $e=$st->error; $st->close(); jsonResponse(["success"=>false,"message"=>"Could not create fee due: ".$e],500); }
    $feeId=(int)$st->insert_id; $st->close();

    // Verify in the same request.
    $v=$conn->prepare("SELECT id, amount, status FROM member_fees WHERE id=? AND member_id=? LIMIT 1");
    if(!$v){jsonResponse(["success"=>false,"message"=>"Fee due verification failed: ".$conn->error],500);}
    $v->bind_param("ii",$feeId,$memberId); $v->execute(); $check=$v->get_result()->fetch_assoc(); $v->close();
    if(!$check || $check['status']!=='Pending') jsonResponse(["success"=>false,"message"=>"Fee due was not saved as Pending."],500);

    jsonResponse(["success"=>true,"message"=>"Initial fee due created.","id"=>$feeId,"amount"=>(float)$check['amount'],"status"=>$check['status']]);
}
