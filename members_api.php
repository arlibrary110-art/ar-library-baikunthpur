...
if ($action === "add") {
    ...
    /*
    |---------------------------------------------------------
    | CREATE INITIAL FEE DUE
    |---------------------------------------------------------
    | IMPORTANT: the MongoDB adapter uses the canonical member_fees
    | collection. Do not create/alter this table here; the adapter
    | already owns its schema. Fail loudly if the due row cannot be
    | created instead of returning success while silently losing the due.
    */
    ...
    $feeAmount = (float)($feeDefaults[$feeKey] ?? 0);
    ...
    if ($feeStmt) {
        ...
        if ($feeStmt->execute()) {
            ...
            if ($feeRow && isset($feeRow["setting_value"])) {
                $feeAmount = (float)$feeRow["setting_value"];
            }
        }
        $feeStmt->close();
    }
    ...
    if ($feeAmount <= 0) {
        $conn->query("INSERT OR IGNORE INTO fee_settings(setting_key,setting_value) VALUES('" . $conn->real_escape_string($feeKey) . "'," . (float)($feeDefaults[$feeKey] ?? 0) . ")");
        $feeAmount = (float)($feeDefaults[$feeKey] ?? 0);
    }
    ...
    if (!$dueStmt) {
        $error = $conn->error ?: "Could not prepare initial fee due.";
        $cleanup = $conn->prepare("DELETE FROM members WHERE id = ? LIMIT 1");
        if ($cleanup) { $cleanup->bind_param("i", $newId); $cleanup->execute(); $cleanup->close(); }
        jsonResponse(["success" => false, "message" => "Could not create initial fee due: " . $error], 500);
    }

    $dueStmt->bind_param("isdsss", $newId, $dueReceipt, $feeAmount, $joining_date, $joining_date, $remark);
    if (!$dueStmt->execute()) {
        $error = $dueStmt->error ?: $conn->error ?: "Could not save initial fee due.";
        $dueStmt->close();
        $cleanup = $conn->prepare("DELETE FROM members WHERE id = ? LIMIT 1");
        if ($cleanup) { $cleanup->bind_param("i", $newId); $cleanup->execute(); $cleanup->close(); }
        jsonResponse(["success" => false, "message" => "Could not create initial fee due: " . $error], 500);
    ...
    // Verify the due row exists in the current request before reporting success.
    $verifyDue = $conn->prepare("SELECT id, amount, status FROM member_fees WHERE member_id = ? AND status = 'Pending' ORDER BY id DESC LIMIT 1");
    if (!$verifyDue) {
        jsonResponse(["success" => false, "message" => "Member was saved but initial fee due could not be verified."], 500);
    }
    $verifyDue->bind_param("i", $newId);
    $verifyDue->execute();
    $dueCheck = $verifyDue->get_result()->fetch_assoc();
    $verifyDue->close();
    if (!$dueCheck) {
        $cleanup = $conn->prepare("DELETE FROM members WHERE id = ? LIMIT 1");
        if ($cleanup) { $cleanup->bind_param("i", $newId); $cleanup->execute(); $cleanup->close(); }
        jsonResponse(["success" => false, "message" => "Could not verify initial fee due. Member was not kept."], 500);
    }
