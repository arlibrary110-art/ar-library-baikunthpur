...
if ($action === "add") {
    ...
    /*
    |---------------------------------------------------------
    | PREPARE FEE SETTINGS BEFORE STARTING THE MEMBER TXN
    |---------------------------------------------------------
    | The old flow inserted the member first and then attempted
    | to create the initial Pending fee. If that second query
    | failed, the UI showed an error even though the member had
    | already been saved. We now make member + initial due one
    | atomic operation.
    */
    $feeTable = "
    ...
    ";
    if (!$conn->query($feeTable)) {
        jsonResponse(["success"=>false,"message"=>"Could not prepare fee settings: ".$conn->error],500);
    }
    ...
    $feeIns = $conn->prepare("INSERT IGNORE INTO fee_settings(setting_key,setting_value) VALUES(?,?)");
    if (!$feeIns) {
        jsonResponse(["success"=>false,"message"=>"Could not prepare fee settings: ".$conn->error],500);
    }
    foreach ($feeDefaults as $fk => $fv) {
        $feeIns->bind_param("sd", $fk, $fv);
        $feeIns->execute();
    }
    $feeIns->close();
    ...
    if (strpos($planKey, "6 month") !== false) {
        ...
        /* Keep compatibility with the existing fee settings. */
        $termKey = "six";
    } elseif (strpos($planKey, "2 month") !== false) {
        $termKey = "one";
    }
    ...
    if ($feeAmount <= 0) {
        jsonResponse(["success"=>false,"message"=>"Initial fee amount is not configured for this plan/shift. Please check Fee Settings."],422);
    }

    /* Make sure older databases have every column used below. */
    $feeColumns = [
    ...
    foreach ($feeColumns as $column => $definition) {
        $check = $conn->query("SHOW COLUMNS FROM member_fees LIKE '" . $conn->real_escape_string($column) . "'");
        if ($check && $check->num_rows === 0) {
            /* CURRENT_DATE default is not accepted on some older MySQL versions. */
            if ($column === "payment_date") {
                $definition = "DATE DEFAULT NULL";
            }
            if (!$conn->query("ALTER TABLE member_fees ADD COLUMN $column $definition")) {
                jsonResponse(["success"=>false,"message"=>"Could not prepare fee table: ".$conn->error],500);
            }
        }
    }
    ...
    try {
        $stmt = $conn->prepare("
            INSERT INTO members
            (member_id,name,phone,email,membership_plan,shift,joining_date,validity_date,date_of_birth,address,status)
            VALUES (?,?,?,?,?,?,?,?,?,?,?)
        ");
        if (!$stmt) throw new Exception("Could not prepare member save: " . $conn->error);
        $stmt->bind_param("sssssssssss", $member_id,$name,$phone,$email,$membership_plan,$shift,$joining_date,$validity_date,$date_of_birth,$address,$status);
        if (!$stmt->execute()) throw new Exception("Could not add member: " . $stmt->error);
        $newId = $stmt->insert_id;
        $stmt->close();

        /* Initial registration fee: always one explicit Pending row. */
        ...
        ");
        if (!$dueStmt) throw new Exception("Could not prepare initial fee due: " . $conn->error);
        $dueStmt->bind_param("isdsss", $newId,$dueReceipt,$feeAmount,$joining_date,$joining_date,$remark);
        if (!$dueStmt->execute()) throw new Exception("Could not save initial fee due: " . $dueStmt->error);
        $dueStmt->close();

        $conn->commit();
        jsonResponse([
            "success"=>true,
            "message"=>"Member added successfully. Initial fee due added.",
            "id"=>$newId,
            "fee_due"=>$feeAmount
        ]);
    } catch (Throwable $e) {
