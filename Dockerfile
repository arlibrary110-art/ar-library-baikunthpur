...
# Google Sheets adapter uses SQLite3 as a temporary SQL engine.
# mysqli/PDO_SQLite are not required by the live application.
RUN apt-get update \
...
# Enable PHP OPcache settings when the bundled extension is available.
RUN { \
...
# Render provides PORT at runtime; keep Apache on the configured container port.
