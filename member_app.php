...
function render(data){
 ...
 document.getElementById('profile').innerHTML=`
  ...
  <div class="item"><b>Status</b><span class="badge">${esc(m.status||'Active')}</span></div>`;
...
async function startScanner(){
 ...
 }catch(e){console.error(e);msg('Camera could not start. Please allow camera permission.');document.getElementById('scanBtn').classList.remove('hidden');document.getElementById('reader').classList.add('hidden');document.getElementById('stopBtn').classList.add('hidden');scanner=null;}
}
...
loadMe();
if(initialQR){setTimeout(()=>sendScan(initialQR),400);}
</script>
</body>
</html>
