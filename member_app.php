...
<style>
*{box-sizing:border-box}body{margin:0;font-family:Arial,sans-serif;background:#f4f6fb;color:#26365b}.wrap{max-width:680px;margin:auto;padding:16px}.top{background:#26365b;color:#fff;border-radius:20px;padding:20px;margin-bottom:14px;display:flex;justify-content:space-between;align-items:center}.top h1{margin:0;font-size:23px}.top small{opacity:.8}.logout{border:0;border-radius:9px;padding:9px 12px;background:#fff;color:#26365b;font-weight:700}.notif-btn{position:relative;border:0;border-radius:10px;padding:9px 11px;background:#fff;color:#26365b;font-size:19px;font-weight:800}.notif-badge{position:absolute;top:-6px;right:-6px;min-width:19px;height:19px;padding:2px 5px;border-radius:10px;background:#d92d20;color:#fff;font-size:11px;line-height:15px}.notif-panel{display:none;margin-top:10px}.notif-panel.show{display:block}.notif-item{display:flex;gap:10px;padding:12px;border-radius:12px;background:#f7f8fb;margin-bottom:9px;border-left:4px solid #26365b}.notif-item.priority-1{border-left-color:#d92d20}.notif-item.priority-2{border-left-color:#f79009}.notif-item.priority-3{border-left-color:#fdb022}.notif-icon{font-size:20px}.notif-title{font-weight:800;margin-bottom:3px}.notif-text{font-size:13px;color:#667085;line-height:1.4}.card{background:#fff;border-radius:18px;padding:18px;margin-bottom:14px;box-shadow:0 6px 22px rgba(0,0,0,.06)}.profile-grid{display:grid;grid-template-columns:1fr 1fr;gap:12px}.item{background:#f7f8fb;padding:12px;border-radius:12px}.item b{display:block;font-size:12px;color:#667085;margin-bottom:4px}.scan{width:100%;padding:16px;border:0;border-radius:14px;background:#26365b;color:#fff;font-size:17px;font-weight:800}.stop{background:#bf5268}.msg{margin-top:12px;padding:12px;border-radius:12px;background:#f1f5f9;text-align:center;font-weight:700}.hidden{display:none!important}#reader{width:100%;margin-top:14px}.history{width:100%;border-collapse:collapse;font-size:14px}.history th,.history td{padding:10px 6px;border-bottom:1px solid #e5e7eb;text-align:left}.badge{display:inline-block;padding:5px 8px;border-radius:8px;background:#e8eef8;font-size:12px;font-weight:700}.empty{text-align:center;color:#667085;padding:15px}.qr-note{font-size:13px;color:#667085;line-height:1.5}
@media(max-width:520px){.profile-grid{grid-template-columns:1fr}.top{align-items:flex-start}.top h1{font-size:20px}}
</style>
</head>
<body>
<div class="wrap">
<div class="top"><div style="display:flex;align-items:center;gap:10px"><img src="assets/ar-library-logo.webp" alt="AR Library logo" style="width:46px;height:46px;object-fit:contain;border-radius:10px"><div><h1>AR LIBRARY</h1><small>Member App</small></div></div><div style="display:flex;align-items:center;gap:8px"><button class="notif-btn" onclick="toggleNotifications()" aria-label="Notifications">🔔<span id="notifBadge" class="notif-badge hidden">0</span></button><button class="logout" onclick="logout()">Logout</button></div></div>

<div id="notifPanel" class="card notif-panel"><h3 style="margin-top:0">🔔 Notifications</h3><div id="notifications"><div class="empty">Loading...</div></div></div>
...
function renderNotifications(list){
 ...
 if(!Array.isArray(list)||!list.length){box.innerHTML='<div class="empty">No new notifications.</div>';badge.classList.add('hidden');badge.textContent='0';return;}
 badge.textContent=list.length>9?'9+':String(list.length);badge.classList.remove('hidden');
 box.innerHTML=list.map(n=>`<div class="notif-item priority-${esc(n.priority||5)}"><div class="notif-icon">${esc(n.icon||'🔔')}</div><div><div class="notif-title">${esc(n.title||'Notification')}</div><div class="notif-text">${esc(n.message||'')}</div></div></div>`).join('');
}
async function loadNotifications(){
 try{
  const r=await fetch('member_api.php?action=notifications',{cache:'no-store'});
  const d=await r.json();
  if(d.success) renderNotifications(d.notifications||[]);
