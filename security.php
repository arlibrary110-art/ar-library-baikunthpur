...
function private_storage_dir() {
    ...
    // In production the Docker image stores private files outside the web root.
    $protected = '/opt/ar_library_private';
    if (is_dir($protected)) {
        $dir = $protected;
    } else {
...
function attendance_qr_secret(): string {
    $configured = getenv('ATTENDANCE_QR_SECRET');
    if ($configured !== false && trim((string)$configured) !== '') return trim((string)$configured);
    $mongo = getenv('MONGODB_URI');
    return hash('sha256', 'AR_LIBRARY_ATTENDANCE_SECRET|' . (string)$mongo);
}

function attendance_qr_token(): string {
    // Permanent office QR: the printed QR never expires or changes.
    // The signature is deterministic for the configured library secret.
    $payload = 'AR_LIBRARY_PERMANENT_ATTENDANCE_QR_V1';
    $sig = hash_hmac('sha256', $payload, attendance_qr_secret());
    return 'permanent.' . substr($sig, 0, 32);
}

function valid_attendance_qr_token(string $token): bool {
    $token = trim($token);
    return $token !== '' && hash_equals(attendance_qr_token(), $token);
}

function app_log($action, $details='') {
    global $conn;
    if (!isset($conn) || !($conn instanceof ARMongoDB) || empty($_SESSION['staff_id'])) return;
...
function ensure_staff_permissions_table() {
    global $conn;
    static $done = false;
    if ($done || !isset($conn) || !($conn instanceof ARMongoDB)) return;
    if (!empty($_SESSION['_permissions_schema_ready'])) { $done = true; return; }
    ...
    $_SESSION['_permissions_schema_ready'] = 1;
    $done = true;
}
