...
if ($action === "notifications") {
    ...
    $feeStmt = $conn->prepare("SELECT COALESCE(SUM(amount),0) AS due_amount FROM member_fees WHERE member_id = ? AND LOWER(TRIM(status)) = 'pending'");
    if ($feeStmt) {
    ...
    $seatStmt = $conn->prepare("SELECT seat_no, shift FROM member_seats WHERE member_id = ? AND LOWER(TRIM(status)) = 'assigned' AND (start_date IS NULL OR start_date = '' OR start_date <= CURDATE()) AND (end_date IS NULL OR end_date = '' OR end_date >= CURDATE()) ORDER BY id DESC LIMIT 1");
    if ($seatStmt) {
...
if ($action === "me") {
    ...
    $seatStmt = $conn->prepare(
        "SELECT seat_no
         FROM member_seats
         WHERE member_id = ?
           AND LOWER(TRIM(status)) = 'assigned'
    ...
    $feeStmt = $conn->prepare(
        "SELECT COALESCE(SUM(amount), 0) AS due_amount
         FROM member_fees
         WHERE member_id = ? AND LOWER(TRIM(status)) = 'pending'"
    );
    if ($feeStmt) {
        $feeStmt->bind_param("i", $id);
        $feeStmt->execute();
