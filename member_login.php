...
<button class="login-btn" type="button" onclick="memberLogin()">Login</button><div class="forgot">Forgot Member ID / Phone?</div><div id="message"></div>
<div class="tag">“बैकुंठपुर की शान,<br>हर विद्यार्थी की पहचान।”</div>
</main>
<script>
async function memberLogin(){const memberId=document.getElementById('memberId').value.trim(),phone=document.getElementById('phone').value.trim(),message=document.getElementById('message');message.textContent='';if(!memberId||!phone){message.textContent='Member ID and phone are required';return;}const form=new FormData();form.append('member_id',memberId);form.append('phone',phone);try{const r=await fetch('member_api.php?action=login',{method:'POST',body:form,cache:'no-store'}),d=await r.json();if(d.success)location.href='member_app.php';else message.textContent=d.message||'Login failed';}catch(e){console.error(e);message.textContent='Server connection error';}}
