...
final class ARMongoDB {
    ...
    private const SCHEMA = [
        ...
        'members' => [
            'columns'=>['id','member_id','name','phone','email','course','membership_plan','shift','joining_date','date_of_birth','validity_date','address','status','created_at'],
            'types'=>['id'=>'INTEGER','member_id'=>'TEXT','name'=>'TEXT','phone'=>'TEXT','email'=>'TEXT','course'=>'TEXT','membership_plan'=>'TEXT','shift'=>'TEXT','joining_date'=>'TEXT','date_of_birth'=>'TEXT','validity_date'=>'TEXT','address'=>'TEXT','status'=>'TEXT','created_at'=>'TEXT'],
    ...
    public function __construct() {
        ...
        // Serialize the load/modify/flush cycle so concurrent Render requests cannot
        // overwrite each other's MongoDB changes through the SQLite compatibility layer.
        $lockPath = '/tmp/ar_library_mongo_sync.lock';
        $this->syncLock = @fopen($lockPath, 'c');
        if ($this->syncLock && !@flock($this->syncLock, LOCK_EX)) {
            @fclose($this->syncLock);
            $this->syncLock = null;
            $this->fail('Could not acquire database synchronization lock. Please retry.');
            return;
        }
        if ($uri === '') {
        ...
        try {
            ...
            register_shutdown_function(function () {
                try { $this->flushDirty(); }
                    ...
                    if ($this->syncLock) { @flock($this->syncLock, LOCK_UN); @fclose($this->syncLock); $this->syncLock = null; }
                }
            });
        } catch (Throwable $e) {
    ...
    public function close(): void { $this->flushDirty();$this->sqlite->close(); if($this->syncLock){@flock($this->syncLock,LOCK_UN);@fclose($this->syncLock);$this->syncLock=null;} }
