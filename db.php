...
final class ARMongoDB {
    ...
    private function writeMongoTable(string $table): void {
        ...
        $session=null;
        try {
            // Preferred path: atomic collection replacement when MongoDB supports transactions.
        ...
        // Fallback for Mongo deployments where transactions are unavailable.
        // This is important on Render when the configured MongoDB endpoint is
        // standalone/non-transactional. The previous code reported a fee-save
        // error even though the SQLite operation itself had succeeded.
        try {
            $collection->deleteMany([]);
            if($docs) $collection->insertMany($docs, ['ordered'=>true]);
        } catch(Throwable $fallbackError) {
            throw new RuntimeException(
                'MongoDB sync failed for '.$table.': '.$fallbackError->getMessage(),
                0,
                $fallbackError
            );
        }
