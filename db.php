...
final class ARMongoDB {
    ...
    private array $dirtyBeforeTransaction = [];
    ...
    public function __construct() {
        ...
        try {
            ...
            // Serialize app requests while using the SQLite snapshot adapter.
            // Without this, two concurrent requests can load the same snapshot
            // and the later full-table sync can overwrite the earlier change.
            $this->acquireRequestLock();
            // Load only the small set needed for bootstrap. Other tables are loaded lazily per SQL request.
            $this->loadMongoTables(['staff','library_settings','fee_settings']);
            $this->seedDefaults();
            register_shutdown_function(function () {
                try { $this->flushDirty(); }
                catch (Throwable $e) { error_log('MongoDB sync failed: '.$e->getMessage()); }
                finally { $this->releaseRequestLock(); }
            });
        } catch (Throwable $e) {
    ...
    private function ensureMongoIndexes(): void {
        $marker = '/tmp/ar_library_mongo_indexes_ready';
    ...
    private function ensureTablesForSql(string $sql): void {
        ...
        if ($needed) $this->loadMongoTables($needed);
    }

    private function loadMongoTables(array $tables): void {
        if ($this->error !== '') return;
        foreach (array_values(array_unique($tables)) as $table) {
            if (!isset(self::SCHEMA[$table]) || isset($this->loadedTables[$table])) continue;
        ...
        $this->loaded = count($this->loadedTables) === count(self::SCHEMA);
    }
    ...
    private function acquireRequestLock(): void {
        $this->lockOwner = bin2hex(random_bytes(16));
        $locks = $this->database->selectCollection('__ar_library_locks');
        $now = new \MongoDB\BSON\UTCDateTime((int)(microtime(true) * 1000));
        $expires = new \MongoDB\BSON\UTCDateTime((int)((microtime(true) + 120) * 1000));
        for ($attempt = 0; $attempt < 30; $attempt++) {
            try {
                $result = $locks->findOneAndUpdate(
                    ['_id' => 'app', '$or' => [
                        ['expires_at' => ['$lte' => $now]],
                        ['owner' => $this->lockOwner]
                    ]],
                    ['$set' => ['owner' => $this->lockOwner, 'expires_at' => $expires]],
                    ['upsert' => true, 'returnDocument' => \MongoDB\Operation\FindOneAndUpdate::RETURN_DOCUMENT_AFTER]
                );
                if ($result) { $this->lockHeld = true; return; }
            } catch (\MongoDB\Driver\Exception\BulkWriteException $e) {
                // Another request may have won the upsert race; wait and retry.
            } catch (Throwable $e) {
        ...
        throw new RuntimeException('Could not acquire database request lock. Please retry.');
    }

    private function refreshRequestLock(): void {
        if (!$this->lockHeld || $this->lockOwner === '') return;
        $locks = $this->database->selectCollection('__ar_library_locks');
        $locks->updateOne(
            ['_id' => 'app', 'owner' => $this->lockOwner],
            ['$set' => ['expires_at' => new \MongoDB\BSON\UTCDateTime((int)((microtime(true) + 120) * 1000))]]
        );
    }

    private function releaseRequestLock(): void {
        if (!$this->lockHeld || $this->lockOwner === '') return;
        try {
            $locks = $this->database->selectCollection('__ar_library_locks');
            $locks->deleteOne(['_id' => 'app', 'owner' => $this->lockOwner]);
        } catch (Throwable $e) {
            error_log('MongoDB lock release failed: '.$e->getMessage());
        }
        $this->lockHeld = false;
    }
    ...
    private function seedDefaults(): void {
        ...
        $settings=['library_name'=>'AR LIBRARY','phone'=>'8349852152','address'=>'सरकारी हॉस्पिटल के सामने, बैकुण्ठपुर (म.प्र.)','opening_time'=>'08:00','closing_time'=>'20:00','morning_end'=>'14:00','evening_start'=>'14:00','total_seats'=>'76','total_lockers'=>'76','monthly_fee'=>'1100','currency'=>'INR','receipt_footer'=>'Thank you for choosing AR Library.','attendance_latitude'=>'24.735323','attendance_longitude'=>'81.409182','attendance_radius_meters'=>'100'];
        ...
        $fees=['half_day_one'=>600,'half_day_three'=>1710,'half_day_six'=>3240,'half_reserved_one'=>800,'half_reserved_three'=>2280,'half_reserved_six'=>4320,'full_day_one'=>1100,'full_day_three'=>3135,'full_day_six'=>5940,'full_reserved_one'=>1300,'full_reserved_three'=>3705,'full_reserved_six'=>7020,'half_day_twelve'=>6480,'half_reserved_twelve'=>8640,'full_day_twelve'=>11880,'full_reserved_twelve'=>14040];
        ...
        // One-time migration of the old shipped defaults only. Custom live values are preserved.
        $oldAddress='Ward No. 15, Sarkari Hospital ke Samane, Baikunthpur, Rewa, Madhya Pradesh - 486441';
        $oldSeats=(string)($this->sqlite->querySingle("SELECT setting_value FROM library_settings WHERE setting_key='total_seats' LIMIT 1")??'');
        $oldLockers=(string)($this->sqlite->querySingle("SELECT setting_value FROM library_settings WHERE setting_key='total_lockers' LIMIT 1")??'');
        if($oldSeats==='79'){$this->sqlite->exec("UPDATE library_settings SET setting_value='76' WHERE setting_key='total_seats' AND setting_value='79'");$this->dirty['library_settings']=true;}
        if($oldLockers==='79'){$this->sqlite->exec("UPDATE library_settings SET setting_value='76' WHERE setting_key='total_lockers' AND setting_value='79'");$this->dirty['library_settings']=true;}
        $addr=(string)($this->sqlite->querySingle("SELECT setting_value FROM library_settings WHERE setting_key='address' LIMIT 1")??'');
        if($addr===$oldAddress){$st=$this->sqlite->prepare("UPDATE library_settings SET setting_value=? WHERE setting_key='address'");$newAddress='सरकारी हॉस्पिटल के सामने, बैकुण्ठपुर (म.प्र.)';$st->bindValue(1,$newAddress);$st->execute();$this->dirty['library_settings']=true;}
    }

    private function flushDirty(): void {
        if($this->error!==''||!$this->dirty) return;
        foreach(array_keys($this->dirty) as $table) $this->writeMongoTable($table);
        $this->dirty=[];
    }

    private function writeMongoTable(string $table): void {
        $this->refreshRequestLock();
        $def=self::SCHEMA[$table];
    ...
    public function begin_transaction(): bool { $this->dirtyBeforeTransaction=$this->dirty; $this->inTransaction=true; return $this->sqlite->exec('BEGIN'); }
    public function commit(): bool { $ok=$this->sqlite->exec('COMMIT'); if(!$ok){$this->inTransaction=false; return false;} $this->inTransaction=false; $this->dirtyBeforeTransaction=[]; $this->refreshRequestLock(); try{$this->flushDirty();}catch(Throwable $e){$this->error=$e->getMessage();$this->errno=1;return false;} return true; }
    public function rollback(): bool { $ok=$this->sqlite->exec('ROLLBACK'); $this->inTransaction=false; $this->dirty=$this->dirtyBeforeTransaction; $this->dirtyBeforeTransaction=[]; return $ok; }
    public function sync(): void { $this->flushDirty(); }
    ...
    public function executePrepared(string $sql,array $refs,array $types,ARMongoStatement $stmt): bool {
        ...
        }else{$stmt->affected_rows=$this->sqlite->changes();$this->affected_rows=$stmt->affected_rows;$this->insert_id=(int)$this->sqlite->lastInsertRowID();$this->markDirtyFromSql($converted); if(!$this->inTransaction){$this->refreshRequestLock();$this->flushDirty();}}
        return true;
    }
    ...
    private function executeSql(string $sql,array $vals,array $types,?ARMongoStatement $stmt){
        ...
        $this->affected_rows=$this->sqlite->changes();$this->insert_id=(int)$this->sqlite->lastInsertRowID();$this->markDirtyFromSql($s);if(!$this->inTransaction){$this->refreshRequestLock();$this->flushDirty();}return true;
    }
