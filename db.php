...
// Non-destructive schema bootstrap. CREATE TABLE IF NOT EXISTS is safe for existing data.
$schemaPath = '/opt/ar_library_private/database.sql';
if (!is_readable($schemaPath)) {
    $schemaPath = __DIR__ . '/AR_Library_Private/database.sql';
}

$requiredTables = ['staff', 'members', 'member_fees', 'payments', 'attendance', 'library_settings'];
$missingTable = false;
...
if ($missingTable && is_readable($schemaPath)) {
    ...
    if ($sql !== false && trim($sql) !== '') {
        // The selected database is already established; do not switch databases.
...
// Permission table is safe to create if missing.
