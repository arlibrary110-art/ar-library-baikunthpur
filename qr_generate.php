...
/* Permanent attendance QR. The QR itself never expires. */
$settings = [];
...
$radius = (int)($settings['attendance_radius_meters'] ?? 100);
if ($radius < 1) $radius = 100;

$scheme = (!empty($_SERVER['HTTPS']) && $_SERVER['HTTPS'] !== 'off') ? 'https' : 'http';
$host = $_SERVER['HTTP_HOST'] ?? 'localhost';
$dir = rtrim(str_replace('\\','/',dirname($_SERVER['SCRIPT_NAME'] ?? '/qr_generate.php')), '/');
$qrUrl = $scheme . '://' . $host . ($dir ? $dir : '') . '/member_app.php?attendance=1';

// Stable marker. Authentication + server-side GPS verification protect attendance.
...
echo json_encode([
    ...
    'message'=>'Permanent attendance QR. Location verification is performed on the server.'
