...
async function addMember(event) {
    ...
    try {
        ...
        if (data.success) {
            ...
            // Member is saved first. Create its initial Pending fee in a
            // separate request so a fee-ledger error cannot be reported as
            // a false "Could not save member" message.
            try {
                const dueForm = new FormData();
                dueForm.append("member_id", data.id);
                const dueResponse = await fetch("members_api.php?action=create_initial_due", { method: "POST", body: dueForm });
                const dueText = await dueResponse.text();
                let dueData = null;
                try { dueData = JSON.parse(dueText); } catch (_) {}
                if (!dueResponse.ok || !dueData || !dueData.success) {
                    console.error("Initial fee due error:", dueText);
                    toast("Member saved, but Fee Due could not be created");
                } else {
                    toast("Member added + Fee Due created");
                }
            } catch (feeError) {
                console.error("Initial fee due request error:", feeError);
                toast("Member saved, but Fee Due could not be created");
            }
