                ...
                <div class="panel member-add-card hidden" id="memberFormCard">
                    ...
                    <form id="memberForm" onsubmit="addMember(event)">
                        <div class="form-grid">
                            ...
                            <div class="field"><label>Membership Plan *</label><select id="m_plan" onchange="updateValidityDate()"><option value="1 Month">1 Month</option><option value="3 Months">3 Months</option><option value="6 Months">6 Months</option><option value="Custom Date">Custom Date</option></select></div>
        ...
        html: () => `
            <div class="fees-module">
                ...
                <div id="feeTabIncome" class="fee-tab-panel hidden"><div class="panel">
                    ...
                    <div class="table-wrap"><table class="table"><thead><tr><th>Date</th><th>Transactions</th><th>Total Income</th></tr></thead><tbody id="incomeBody"><tr><td colspan="3" class="empty">Loading...</td></tr></tbody></table></div>
                </div></div>
    ...
    body.innerHTML=rows.map(member=>{
        ...
        return `<article class="member-profile-card">
            ...
            <div class="member-card-footer">
                ...
                <div class="member-row-actions"><button class="member-row-btn view" type="button" onclick="viewMember(${Number(member.id)})">View</button><button class="member-row-btn edit" type="button" onclick="editMember(${Number(member.id)})">Edit</button>${String(document.body.dataset.role||'').toLowerCase()==='admin'?`<button class="member-row-btn delete" type="button" onclick="deleteMember(${Number(member.id)}, '${escapeJs(member.name)}')">Delete</button>`:''}</div>
            </div>
        </article>`;
    }).join("");
}
...
async function loadSeats(){
    try{
        ...
        const total=Math.max(1,Number(d.total_seats||79));
        ...
        renderLockersPro(d.lockers||[],Math.max(1,Number(d.total_lockers||79)));
    }catch(e){toast(e.message)}
}
...
function feeMonthDefault(){return new Date().toISOString().slice(0,7)}
async function loadMonthlyIncome(){
    ...
    try{const d=await apiRequest('payments_api.php?action=monthly_income&month='+encodeURIComponent(month?.value||feeMonthDefault()));
        body.innerHTML=(d.days||[]).map(x=>`<tr><td>${esc(x.payment_date)}</td><td>${esc(x.transactions)}</td><td>${money(x.total_income)}</td></tr>`).join('')||'<tr><td colspan="3" class="empty">No income found for this month.</td></tr>';
        const el=document.getElementById('incomeSummary'); if(el) el.textContent=`Total income: ${money(d.total_income||0)} · ${d.total_transactions||0} transactions`;
...
async function loadFeeSettings(){
    ...
    try{const d=await apiRequest('payments_api.php?action=fee_settings');
        ...
        box.innerHTML=`<table class="table fee-settings-table"><thead><tr><th>Plan</th><th>1 Month</th><th>3 Months (5% Off)</th><th>6 Months (10% Off)</th></tr></thead><tbody>${rows.map(([key,label])=>`<tr><td><b>${label}</b></td><td><input class="fee-setting-input" data-key="${key}" data-term="1" type="number" min="0" step="1" value="${Number(r[key]?.one_month||0)}"></td><td><input class="fee-setting-input" data-key="${key}" data-term="3" type="number" min="0" step="1" value="${Number(r[key]?.three_month||0)}"></td><td><input class="fee-setting-input" data-key="${key}" data-term="6" type="number" min="0" step="1" value="${Number(r[key]?.six_month||0)}"></td></tr>`).join('')}</tbody></table><div class="note">Current structure: ₹600 / ₹800 / ₹1100 / ₹1300 monthly; 3 months at 5% off; 6 months at 10% off.</div>`;
    }catch(e){box.innerHTML=`<div class="empty">${esc(e.message)}</div>`}
}
async function saveFeeSettings(){
    const inputs=[...document.querySelectorAll('.fee-setting-input')]; const settings={};
    inputs.forEach(i=>{settings[i.dataset.key]??={}; const n=Number(i.value); settings[i.dataset.key][i.dataset.term=== '1'?'one_month':i.dataset.term==='3'?'three_month':'six_month']=isFinite(n)?n:0});
...
async function sendFeeReminder(id){
    ...
    let libraryAddress = 'Ward No. 15, Sarkari Hospital ke Samane, Baikunthpur, Rewa, Madhya Pradesh - 486441';
    try {
...
async function openPaymentForm(prefill={}){
    try {
        ...
        modal('Collect / Record Payment', `<div class="form-grid">
            ...
            <div class="field"><label>Duration</label><select id="pay_duration"><option value="1">1 Month</option><option value="3">3 Months — 5% Off</option><option value="6">6 Months — 10% Off</option></select></div>
