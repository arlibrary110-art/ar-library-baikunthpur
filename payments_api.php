...
if ($action === "monthly_income") {
    ...
    $stmt = $conn->prepare("SELECT payment_date,COUNT(*) transactions,COALESCE(SUM(amount),0) total_income FROM payments WHERE payment_date BETWEEN ? AND ? GROUP BY payment_date ORDER BY payment_date DESC");
...
if ($action === "add") {
    ...
    */
    ...
    /* Keep the member profile fee history in sync with the main Fees module. */
    $sync = $conn->prepare("
    ...
    if ($sync) {
        ...
        $sync->bind_param("isdsss", $memberId, $receiptNo, $amount, $paymentDate, $paymentMethod, $syncRemark);
        $sync->execute();
        $sync->close();
    }
