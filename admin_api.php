...
if($action==='dashboard'){
 ...
 $totalSeats=(int)get_setting('total_seats',76); if($totalSeats<1)$totalSeats=76; $availableSeats=max(0,$totalSeats-$occupied);
...
if($action==='seats'){
 ...
 $totalSeats=(int)get_setting('total_seats',76);
 $totalLockers=(int)get_setting('total_lockers',$totalSeats);
 if($totalSeats<1)$totalSeats=76; if($totalLockers<1)$totalLockers=$totalSeats;
...
if($action==='assign_seat' && $method==='POST'){
 ...
 $totalSeats=(int)get_setting('total_seats',76); if($totalSeats<1)$totalSeats=76; if(!ctype_digit($seat) || (int)$seat<1 || (int)$seat>$totalSeats) out(false,'Invalid seat number. Choose a seat from 1 to '.$totalSeats.'.',[],422);
...
if($action==='assign_locker' && $method==='POST'){
 ...
 $totalLockers=(int)get_setting('total_lockers',get_setting('total_seats',76)); if($totalLockers<1)$totalLockers=76; if($locker===''||$member==='')out(false,'Locker number and member are required.',[],422); if(!ctype_digit($locker) || (int)$locker<1 || (int)$locker>$totalLockers) out(false,'Invalid locker number. Choose a locker from 1 to '.$totalLockers.'.',[],422);$mid=0;
...
if($action==='enquiries'){
 ...
 if($method==='POST' && isset($_GET['convert'])){
  ...
  if($valid==='') { $months=$plan==='12 Months'?12:($plan==='6 Months'?6:($plan==='3 Months'?3:1)); $valid=date('Y-m-d',strtotime($join.' +'.$months.' month -1 day')); }
  ...
  try{
   ...
   // Converted enquiries follow the same Member + Initial Fee Due rule as direct registration.
   $shiftKey=strtolower($shift);
   $feePlanKey=strpos($shiftKey,'full day')!==false ? (strpos($shiftKey,'reserved')!==false?'full_reserved':'full_day') : (strpos($shiftKey,'reserved')!==false?'half_reserved':'half_day');
   $termKey=$plan==='12 Months'?'twelve':($plan==='6 Months'?'six':($plan==='3 Months'?'three':'one'));
   ...
   if($feeAmount<=0){$monthly=['half_day'=>600,'half_reserved'=>800,'full_day'=>1100,'full_reserved'=>1300];$feeAmount=$monthly[$feePlanKey]??1100;if($plan==='3 Months')$feeAmount*=3*0.95;elseif($plan==='6 Months')$feeAmount*=6*0.90;elseif($plan==='12 Months')$feeAmount*=12*0.90;}
   if($feeAmount<=0) throw new Exception('Could not determine initial fee due amount.');
   $dueReceipt='DUE-'.$newId.'-'.date('YmdHis').'-'.random_int(100,999);
   $d=$conn->prepare("INSERT INTO member_fees(member_id,receipt_no,amount,payment_date,due_date,payment_method,status,remarks) VALUES(?,?,?,?,?,'Pending','Pending',?)");
   if(!$d) throw new Exception('Could not prepare initial fee due.');
   $remark='Initial registration fee due';
   $d->bind_param('isdsss',$newId,$dueReceipt,$feeAmount,$join,$join,$remark);
   if(!$d->execute()) throw new Exception('Could not create initial fee due: '.$d->error);$d->close();
   log_activity('Converted enquiry',$enq['name'].' → '.$memberId);$conn->commit();$conn->sync();out(true,'Enquiry converted to member. Initial Fee Due created.',['member_id'=>$memberId,'member_db_id'=>$newId,'due_amount'=>$feeAmount]);
...
if($action==='settings'){
 ...
 if($method==='GET'){
  ...
  $data=[];$r=$conn->query('SELECT setting_key,setting_value FROM library_settings');while($x=$r->fetch_assoc())$data[$x['setting_key']]=$x['setting_value']; if(empty($data['address'])) if(empty($data['address'])) $data['address']='सरकारी हॉस्पिटल के सामने, बैकुण्ठपुर (म.प्र.)'; out(true,'',['settings'=>$data]); }
