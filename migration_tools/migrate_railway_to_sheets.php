<?php
/**
 * ONE-TIME Railway/MySQL -> Google Sheets migration.
 *
 * Safety: this endpoint is disabled unless ENABLE_RAILWAY_MIGRATION=1.
 * After a successful migration, remove that environment variable and all
 * OLD_DB_* variables from Render.
 */
if (PHP_SAPI !== 'cli') {
...
if ($host==='' || $user==='' || $pass===false) {
    fwrite(STDERR, "Set OLD_DB_HOST, OLD_DB_PORT, OLD_DB_NAME, OLD_DB_USER and OLD_DB_PASSWORD.\n");
    exit(3);
}
...
$tables = [
 'staff'=>['id','staff_id','name','photo','password','role','status','created_at'],
...
foreach($tables as $table=>$columns){
    ...
    // Clear the Sheets-side copy first, then insert the complete Railway snapshot.
    ...
    while($row=$rs->fetch_assoc()){
        ...
        foreach($columns as $c){
            $v=$row[$c] ?? null;
            if(in_array($c,['id','member_id'],true)) $types.='i';
            elseif(in_array($c,['amount','fee_amount','additional_charges','setting_value'],true) && is_numeric($v)) $types.='d';
            else $types.='s';
            $vals[]=$v;
        }
...
$old->close();
echo json_encode(['success'=>true,'message'=>'Railway data copied to Google Sheets. Remove ENABLE_RAILWAY_MIGRATION and OLD_DB_* variables now.','tables'=>$counts], JSON_UNESCAPED_UNICODE|JSON_UNESCAPED_SLASHES)."\n";
