...
// Backward-compatible migration: add staff profile photo column if missing.
...
if($action==='fresh_start_reset') {
    ...
    try {
        ...
        foreach(['attendance','member_seats','lockers','payments','member_fees','members','enquiries','activity_logs'] as $table){
...
if($action==='dashboard'){
 ...
 $conn->query("ALTER TABLE members ADD COLUMN IF NOT EXISTS date_of_birth DATE NULL");
 $r=$conn->query("SELECT COUNT(*) c FROM members"); $members=(int)$r->fetch_assoc()['c'];
 $r=$conn->query("SELECT COUNT(*) c FROM members WHERE status='Active' AND (validity_date IS NULL OR validity_date>=CURDATE())"); $active=(int)$r->fetch_assoc()['c'];
 $r=$conn->query("SELECT COUNT(*) c FROM members WHERE status='Active' AND validity_date BETWEEN CURDATE() AND DATE_ADD(CURDATE(),INTERVAL 7 DAY)"); $expiring=(int)$r->fetch_assoc()['c'];
 $r=$conn->query("SELECT COUNT(*) c FROM members WHERE status='Active' AND validity_date<CURDATE()"); $expired=(int)$r->fetch_assoc()['c'];
 $r=$conn->query("SELECT COUNT(*) c FROM attendance WHERE attendance_date=CURDATE() AND check_in IS NOT NULL"); $present=(int)$r->fetch_assoc()['c'];
 $r=$conn->query("SELECT COUNT(*) c FROM attendance WHERE attendance_date=CURDATE() AND check_in IS NOT NULL AND check_out IS NULL"); $checkedIn=(int)$r->fetch_assoc()['c'];
 $r=$conn->query("SELECT COALESCE(SUM(amount),0) t FROM payments WHERE payment_date=CURDATE()"); $todayPaid=(float)$r->fetch_assoc()['t'];
 $r=$conn->query("SELECT COALESCE(SUM(amount),0) t FROM payments WHERE YEAR(payment_date)=YEAR(CURDATE()) AND MONTH(payment_date)=MONTH(CURDATE())"); $monthPaid=(float)$r->fetch_assoc()['t'];
 $r=$conn->query("SELECT COALESCE(SUM(amount),0) t FROM expenses WHERE YEAR(expense_date)=YEAR(CURDATE()) AND MONTH(expense_date)=MONTH(CURDATE())"); $monthExp=(float)$r->fetch_assoc()['t'];
 $monthNet=$monthPaid-$monthExp;
 $r=$conn->query("SELECT COUNT(*) c FROM member_seats WHERE status='Assigned' AND (end_date IS NULL OR end_date>=CURDATE())"); $occupied=(int)$r->fetch_assoc()['c'];
 $totalSeats=(int)get_setting('total_seats',79); if($totalSeats<1)$totalSeats=79; $availableSeats=max(0,$totalSeats-$occupied);
 $r=$conn->query("SELECT COUNT(*) c FROM lockers WHERE status='Assigned' AND (end_date IS NULL OR end_date>=CURDATE())"); $occupiedLockers=(int)$r->fetch_assoc()['c'];
 ...
 $r=$conn->query("SELECT COUNT(*) c FROM enquiries WHERE status IN ('Open','Follow-up')"); $openEnq=(int)$r->fetch_assoc()['c'];
 $r=$conn->query("SELECT COUNT(*) c FROM enquiries WHERE status='Converted' AND YEAR(created_at)=YEAR(CURDATE()) AND MONTH(created_at)=MONTH(CURDATE())"); $convertedEnq=(int)$r->fetch_assoc()['c'];
 $r=$conn->query("SELECT COALESCE(SUM(amount),0) t FROM member_fees WHERE status='Pending'"); $pendingFees=$r ? (float)$r->fetch_assoc()['t'] : 0.0;
...
if($action==='seats'){
 ...
 $conn->query("CREATE TABLE IF NOT EXISTS lockers (
   ...
   INDEX idx_locker_member(member_id)
 ) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4");
